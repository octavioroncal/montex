...
Migrations for the app environment live in this folder, and use the local migration runner in
`tools/migrations/scripts/runner.mjs`.

We have an npm script which wraps the runner: `npm run migrations -- ...`
...
When invoking migrations, we specify the relevant tags with the `-t` or `--tags` flag.
...
`migrate` and `rollback` methods, which are run by the migrations CLI when running the migrations. `rollback` should
...
The `-t` flag also works with `rollback`, and `list`.
...
**Note:** Running `rollback` without any arguments rolls back _all_ executed migrations, which you may well not want.
